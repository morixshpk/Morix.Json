...
namespace Morix.Json
{
    ...
    public sealed class JsonSerializationException : Exception
    {
        ...
        public JsonSerializationException()
            : base(GetDefaultMessage(JsonErrorType.Unknown)) { }
        ...
        public JsonSerializationException(JsonErrorType type)
        ...
        /// <param name="type">The error type that describes the cause of the error.</param>
        public JsonSerializationException(string message, JsonErrorType type)
            : base(message)
        {
            this.Type = type;
        }

        private static string GetDefaultMessage(JsonErrorType type)
        {
            switch (type)
            {
                case JsonErrorType.InvalidNumber:
                    return "The value been serialized contains an invalid number value (NAN, infinity).";

                case JsonErrorType.InvalidValueType:
