...
{
    using JsonErrorType = JsonParseException.JsonErrorType;
    ...
    internal sealed class TextScanner
    {
        ...
        public char Peek()
        {
            ...
            if (next == -1)
            {
                throw new JsonParseException(
                    JsonErrorType.IncompleteMessage,
                    this._position
                );
            }
        ...
        public void Assert(char next)
        {
            ...
            else
            {
                throw new JsonParseException(
                    ...
                    JsonErrorType.InvalidOrUnexpectedCharacter,
                    this._position
                );
            }
        }
        ...
        public void Assert(string next)
        {
            ...
            catch (JsonParseException e) when (e.Type == JsonErrorType.InvalidOrUnexpectedCharacter)
            {
                throw new JsonParseException(
                    string.Format("Parser expected '{0}'", next),
                    JsonErrorType.InvalidOrUnexpectedCharacter,
